...
// Resolver provides equivilent functionality to the net.Resolver with one exception - it allows to use a provided DNS server instead of relying on the existing default resolver.
...
func (p *Resolver) LookupSRV(ctx context.Context, service, proto, name string) (cname string, addrs []*net.SRV, err error) {
	p.resolver.Dial = p.resolverDial
	p.resolver.PreferGo = true
